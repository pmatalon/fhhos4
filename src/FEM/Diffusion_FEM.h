...
public:
	...
	void Assemble(const ActionsArguments& actions, const ExportModule& out)
	{
		...
		parallelLoop.Execute([this, actions](Element<Dim>* e, ParallelChunk<CoeffsChunk>* chunk)
			{
				vector<Vertex*> vertices = e->Vertices();
		...
		SparseMatrix A_ii = globalA.topLeftCorner(_mesh->InteriorVertices.size(), _mesh->InteriorVertices.size());
		SparseMatrix A_id = globalA.topRightCorner(_mesh->InteriorVertices.size(), _mesh->DirichletVertices.size());

		this->A = std::move(A_ii);
		...
		if (actions.AssembleRightHandSide)
		{
			...
			this->x_d  = AssembleDirichletTerm(TestCase->BC.DirichletFunction);
	...
	Vector AssembleSourceTerm(DomFunction sourceFunction)
	{
		...
		parallelLoop.Execute([this, &sourceFunction, &b_i](Element<Dim>* e)
			{
				bool boundaryElement = e->IsOnBoundary();
				vector<Vertex*> vertices = e->Vertices();
				for (int i = 0; i < vertices.size(); i++)
				{
					Vertex* v = vertices[i];
	...
	{
		Vector x(x_i.rows() + this->x_d.rows());
		x << x_i, this->x_d;
		return x;
	}
public:
