...
public:
	...
	{
		DoublePairwiseAggregation(coarsestPossibleMeshReached);
	}
...
private:
	void BuildAlgebraicMesh()
	{
		...
		parallelLoopElem.Execute([this](BigNumber elemNumber, ParallelChunk<EmptyResultChunk>* chunk)
			{
	...
	void DoublePairwiseAggregation(bool& coarsestPossibleMeshReached)
	{
		/*CondensedLevel levelToRemove(0, _cellBlockSize, _faceBlockSize);
		levelToRemove.Aggregation(coarsestPossibleMeshReached);
		if (coarsestPossibleMeshReached)
			return;
		levelToRemove.SetupProlongation();*/

		Aggregation(coarsestPossibleMeshReached);
	}


	void Aggregation(bool& coarsestPossibleMeshReached)
	{
		BuildAlgebraicMesh();
...
public:
	...
	void SetupProlongation() override
	{
		...
		if (!inv_A_T_T)
			inv_A_T_T = new SparseMatrix(InvertBlockDiagMatrix(*A_T_T, _cellBlockSize));

		this->A_T_Tc = Q_T.transpose() * (*A_T_T) * Q_T;
		this->A_T_Fc = Q_T.transpose() * (*A_T_F) * Q_F;
		this->A_F_Fc = Q_F.transpose() * (*A_F_F) * Q_F;

		this->inv_A_T_Tc = new SparseMatrix(InvertBlockDiagMatrix(A_T_Tc, _cellBlockSize));
	...
	void OnEndSetup() override
	{
		if (this->CoarserLevel)
		{
			...
			coarse->A_T_T = &this->A_T_Tc;
			coarse->A_T_F = &this->A_T_Fc;
			coarse->A_F_F = &this->A_F_Fc;
			coarse->inv_A_T_T = this->inv_A_T_Tc;
		}
	}
...
public:
	...
	void Setup(const SparseMatrix& A, const SparseMatrix& A_T_T, const SparseMatrix& A_T_F, const SparseMatrix& A_F_F) override
	{
		...
		fine->A_T_F = &A_T_F;
		fine->A_F_F = &A_F_F;
		SparseMatrix* inv_A_T_T = new SparseMatrix(CondensedLevel::InvertBlockDiagMatrix(A_T_T, _cellBlockSize));
