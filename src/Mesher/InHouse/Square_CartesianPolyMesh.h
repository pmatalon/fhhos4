...
class Square_CartesianPolyMesh : public PolyhedralMesh<2>
{
public:
	...
	Square_CartesianPolyMesh(BigNumber nx, BigNumber ny, bool with4Quadrants = false, bool buildMesh = true) : PolyhedralMesh()
	{
	...
	void Build()
	{
		...
		double hx_l = 1.0 / nx_l;
		double hy_l = 1.0 / ny_l;
		double hx_r = 1.0 / nx_r;
		...
		this->Vertices.reserve((nx_l + 1) * (ny_l + 1) + (nx_r + 1) * (ny_r + 1));
		for (BigNumber iy = 0; iy < ny_l + 1; ++iy)
		{
			for (BigNumber ix = 0; ix < nx_l + 1; ++ix)
			{
				Vertex* vertex = new Vertex(indexV_l(ix, iy), ix * hx_l, iy * hy_l);
		...
		for (BigNumber iy = 0; iy < ny_r + 1; ++iy)
		{
			...
			{
				Vertex* vertex = new Vertex(indexV_r(ix, iy), 1.0 + ix * hx_r, iy * hy_r);
				this->Vertices.push_back(vertex);
			}
		}
...
public:
	...
	{
		return "CartesianPolyMesh: " + to_string(this->Nx) + " x " + to_string(this->Ny);
	}
	...
	{
		return new Square_CartesianPolyMesh(this->Nx, this->Ny, this->With4Quadrants);
	}
};
